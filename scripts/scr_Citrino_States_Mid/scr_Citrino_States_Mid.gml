...
function Citrino_Mid_GetTarget() {
...
function Citrino_Mid_State_Idle() {
    ...
    var target = Citrino_Mid_GetTarget();
...
function Citrino_Mid_State_Lighting() {
    ...
    // Se não há alvo ou está longe, volta pro Idle
    if (target == noone || point_distance(x, y, target.x, target.y) > 2000) {
        sprite_index = spr_Citrino_Idle;
        image_index = 0;
        state = Citrino_Mid_State_Idle;
    }
...
function Citrino_Mid_State_Attack() {
    ...
    // Se não há alvo ou ambos já passaram dele
    ...
    if (point_distance(x, y, target.x, target.y) > 1000) {
        sprite_index = spr_Citrino_Lighting;
        image_index = 0;
        state = Citrino_Mid_State_Lighting;
        return;

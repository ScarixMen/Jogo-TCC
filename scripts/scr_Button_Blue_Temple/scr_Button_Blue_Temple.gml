function Button_Blue_Idle_Temple(){
	
	with obj_Bridge
	{
	
		image_index = 0
		
	}
	
	sprite_index = spr_Button_Blue_Idle
	
	if place_meeting(x,y-1,obj_Box)
	{
		image_index = 0
		state = Button_Blue_Pressed_Temple
		
	}
	
}

function Button_Blue_Pressed_Temple(){
	global.cam_override = true
	global.focus_target = instance_nearest(x,y,obj_Box)
	audio_stop_sound(sfx_Push_Box)
	
	if(image_index >= image_number -1)
	{	
		
		 timer --;
		if timer <= 0
		{
			
			global.focus_target = instance_nearest(x,y,obj_Plataform_1)
		
		}
	}
	
	sprite_index = spr_Button_Blue_Press
	
	
	if(image_index >= image_number -1)
	{
		
		image_index = 7
	
	}
	
}